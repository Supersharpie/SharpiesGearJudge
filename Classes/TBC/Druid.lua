...
Druid.Weights = {
    ...
    ["BALANCE_PVE"] = { 
		["MSC_WEAPON_DPS"]                  = 0.0,
        ["ITEM_MOD_HIT_SPELL_RATING_SHORT"] = 1.4, -- Still #1 until capped
        ["ITEM_MOD_SPELL_POWER_SHORT"]      = 1.0, 
        ["ITEM_MOD_ARCANE_DAMAGE_SHORT"]    = 1.0, 
        ["ITEM_MOD_NATURE_DAMAGE_SHORT"]    = 1.0, 
        ["ITEM_MOD_SPELL_CRIT_RATING_SHORT"]= 1.0, -- [FIX] Bumped from 0.8. Vengeance makes Crit huge.
    ...
    ["FERAL_CAT"] = { 
        ...
        ["ITEM_MOD_STRENGTH_SHORT"]         = 2.1, -- 1 Str = 2 AP
        ["ITEM_MOD_AGILITY_SHORT"]          = 1.8, -- 1 Agi = 1 AP + Crit
    ...
    -- [[ 3. FERAL BEAR (Tank) ]]
["FERAL_BEAR"] = { 
        ["MSC_WEAPON_DPS"]                  = 0.0, -- Bears don't use weapon dmg       
        -- SURVIVAL (Effective Health)
        ["ITEM_MOD_STAMINA_SHORT"]          = 1.8, -- King of TBC Tanking
        ["ITEM_MOD_ARMOR_SHORT"]            = 0.18, -- [FIX] Lowered from 0.35 (Armor values are huge)        
        -- MITIGATION & THREAT (The God Stat)
        ["ITEM_MOD_AGILITY_SHORT"]          = 1.6, -- Gives Dodge + Armor + Crit.        
        -- AVOIDANCE
        ["ITEM_MOD_DODGE_RATING_SHORT"]     = 1.4, 
        ["ITEM_MOD_DEFENSE_SKILL_RATING_SHORT"]= 1.3, 
        ["ITEM_MOD_RESILIENCE_RATING_SHORT"]= 1.0, -- Efficient for Cap, useless after       
        -- THREAT (Crucial for TBC)
        ["ITEM_MOD_HIT_RATING_SHORT"]       = 1.5, -- [FIX] Bumped from 0.6. Miss = No Threat.
        ["ITEM_MOD_EXPERTISE_RATING_SHORT"] = 1.6, -- [FIX] Reduces Parry Haste (Defensive)        
        -- OFFENSIVE
        ["ITEM_MOD_STRENGTH_SHORT"]         = 1.0, -- [FIX] 1 Str = 2 AP. Matches Feral AP * 2.
        ["ITEM_MOD_FERAL_ATTACK_POWER_SHORT"]= 0.5,
        ["ITEM_MOD_CRIT_RATING_SHORT"]      = 0.8, -- Savage Defense isn't in TBC, just threat       
        -- TRASH
        ...
        ["ITEM_MOD_PARRY_RATING_SHORT"]     = 0.002, -- Bears cannot Parry
        ["ITEM_MOD_BLOCK_RATING_SHORT"]     = 0.002, -- Bears cannot Block
    },
...
function Druid:ApplyScalers(weights, currentSpec)
    ...
    elseif currentSpec:find("FERAL") or currentSpec:find("Bear") or currentSpec:find("Cat") then
		...
		if (currentSpec:find("FERAL_CAT") or currentSpec:find("Cat")) and weights["ITEM_MOD_ARMOR_PENETRATION_RATING_SHORT"] then
    ...
    elseif currentSpec:find("RESTO") or currentSpec:find("Healer") then
        ...
        if weights["ITEM_MOD_SPIRIT_SHORT"] then
            ...
            -- 1. Get raw MP5 gained from 1 Spirit (using your new helper)
            local mp5Value = MSC:GetSpiritValueInMP5(level, intellect)
            
            -- 2. Combat Regeneration Uptime (Intensity)
            -- Resto Druids have "Intensity" (30% regen while casting).
            -- Druids cast HOTs frequently, so we assume high "Casting" uptime.
            -- Base Multiplier ~ 0.65 (similar to Priest)
            local combatMult = 0.65
            
            -- 3. Living Spirit Bonus (15% Spirit)
            -- We handled this in Step 1, but we should ensure it stacks.
            -- Actually, the helper calculates base regen. Talent multiplier applies to the STAT, not the regen formula directly.
            -- So we leave Living Spirit logic in Step 1 (at the top of the function) and don't double count it here.
            
            -- 4. Convert to Score
            -- If 1 MP5 is worth 2.0 points (defined in Weights), we multiply:
            local mp5Weight = weights["ITEM_MOD_MANA_REGENERATION_SHORT"] or 2.0
    ...
    if (currentSpec:find("FERAL_BEAR") or currentSpec:find("Bear")) and weights["ITEM_MOD_DEFENSE_SKILL_RATING_SHORT"] then
        ...
        -- The Magic Number: Bosses have 5.6% chance to crit.
        local reductionNeeded = 5.6
        
        -- [FIX] Check "Survival of the Fittest" (3% reduction), NOT Thick Hide
        if Rank("SURVIVAL_OF_FITTEST") >= 3 then 
            reductionNeeded = 2.6 
        end
        
        -- Calculate current reduction from stats
        -- Defense: 25 skill = 1% reduction (roughly)
        local defReduction = (defenseSkill - 350) * 0.04
        if defReduction < 0 then defReduction = 0 end
        
        -- Resilience: 39.4 rating = 1% reduction
        local resilReduction = resil / 39.4
        
        local currentReduction = defReduction + resilReduction
        
        -- HYSTERESIS: If we are safely over the cap (+0.2 buffer)
        if currentReduction >= (reductionNeeded + 0.2) then
             weights["ITEM_MOD_DEFENSE_SKILL_RATING_SHORT"] = 0.6
             weights["ITEM_MOD_RESILIENCE_RATING_SHORT"] = 0.5
             table.insert(activeCaps, "Crit Immune")
             
             -- [PIVOT] Once immune, Stamina and Agility/Armor become god stats
             weights["ITEM_MOD_STAMINA_SHORT"] = weights["ITEM_MOD_STAMINA_SHORT"] * 1.2
        ...
        elseif currentReduction >= reductionNeeded then
             ...
             -- WE ARE NOT IMMUNE! Panic mode. Defense/Resil are #1 priority.
             weights["ITEM_MOD_DEFENSE_SKILL_RATING_SHORT"] = 2.5
             weights["ITEM_MOD_RESILIENCE_RATING_SHORT"] = 2.5
        end
    end
