...
function MSC.ParseTooltipLine(text)
    ...
    -- If it's a Set Bonus, only count it if it is GREEN (Active).
    ...
    -- 2. SANITIZE & CLEANUP
    -- Strip color codes so we can check for plain text
    text = text:gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", "")
    
    -- 3. IGNORE TEMPORARY & CONDITIONAL BUFFS
    -- We added "when fighting", "against", and "vs " to catch mob-specific stats.
    if text:find("^Use:") or 
       text:find("^Chance on hit:") or 
       text:find("^Procs:") or
       text:find("when fighting") or    -- Catches "when fighting Elementals/Beasts"
       text:find("against") or          -- Catches "Attack Power against Undead"
       text:find("vs %a+") then         -- Catches "vs Undead" variants
        return nil, 0 
    end
    ...
    local patterns = {
        ...
        -- DEFENSIVE (Moved UP to protect from greedy Hit pattern)
        ...
        { pattern = "Increases your chance to parry an attack by (%d+)%%", stat = "ITEM_MOD_PARRY_RATING_SHORT" },
