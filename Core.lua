...
GameTooltip:HookScript("OnTooltipSetItem", function(tooltip)
    ...
    local slotId = MSC.SlotMap[itemEquipLoc] -- Using Data_Tables.lua
    if not slotId then return end
    
    local currentWeights, specName = MSC.GetCurrentWeights() -- Using Helpers.lua
    ...
    -- COMPARISON CASES (Using Helpers.lua and Scoring.lua)
    if itemEquipLoc == "INVTYPE_2HWEAPON" then
        ...
        if filledFromBag or (equippedMHLoc ~= "INVTYPE_2HWEAPON" and equippedOH) then 
            noteText = "Comparing vs: " .. (mhLink or "Empty") .. " + " .. (ohLink or "Empty") 
        end
    ...
    elseif (itemEquipLoc == "INVTYPE_WEAPON" or itemEquipLoc == "INVTYPE_WEAPONMAINHAND") and equippedMHLoc == "INVTYPE_2HWEAPON" then
        ...
        if bestOHLink then
            newStats = MergeStats(mhStats, bestOHStats); newScore = mhScore + bestOHScore
            partnerItemLink = bestOHLink
        else
            newStats = mhStats; newScore = mhScore; noteText = "|cffff0000(No Offhand found)|r"
        end
        oldStats = MSC.SafeGetItemStats(equippedMH, 16)
        oldScore = MSC.GetItemScore(oldStats, currentWeights, specName, 16)
    ...
    elseif (itemEquipLoc == "INVTYPE_SHIELD" or itemEquipLoc == "INVTYPE_HOLDABLE" or itemEquipLoc == "INVTYPE_WEAPONOFFHAND") and equippedMHLoc == "INVTYPE_2HWEAPON" then
        ...
        local ohScore = MSC.GetItemScore(ohStats, currentWeights, specName, 17)
        if bestMHLink then
            newStats = MergeStats(bestMHStats, ohStats); newScore = bestMHScore + ohScore
            partnerItemLink = bestMHLink
        else
    ...
    elseif itemEquipLoc == "INVTYPE_FINGER" or itemEquipLoc == "INVTYPE_TRINKET" then
        ...
        elseif sc2 < sc1 then targetSlot = s2; targetLink = l2; otherLink = l1
        else targetSlot = s1; targetLink = l1; otherLink = l2 end
    ...
    if isEquipped then
        ...
        if scoreDiff > 0 then tooltip:AddLine("|cff00ff00*** UPGRADE (+" .. string.format("%.1f", scoreDiff) .. ")" .. bestText .. " ***|r")
    ...
    for _, entry in ipairs(sortedDiffs) do
        if entry.key ~= "IS_PROJECTED" then
            ...
            if entry.key == "ITEM_MOD_HEALTH_SHORT" or entry.key == "ITEM_MOD_MANA_SHORT" or entry.key == "ITEM_MOD_ATTACK_POWER_SHORT" 
            or entry.key == "ITEM_MOD_CRIT_RATING_SHORT" or entry.key == "ITEM_MOD_HIT_RATING_SHORT" or entry.key == "ITEM_MOD_WEAPON_SKILL_RATING_SHORT" then isRelevant = true end
            ...
            if isRelevant then
                ...
                if cleanName and cleanName ~= "" then
                    ...
                    if diffVal > 0 then 
                        rightText = "|cffffffff" .. newStr .. "|r |cff00ff00(" .. diffStr .. ")|r"
                        tooltip:AddDoubleLine(cleanName, rightText, 1, 0.82, 0) 
                    elseif diffVal < 0 then 
                        rightText = "|cffffffff" .. newStr .. "|r |cffff0000(" .. diffStr .. ")|r"
                        tooltip:AddDoubleLine(cleanName, rightText, 1, 0.82, 0)
                    elseif newVal > 0 then
                        rightText = "|cffffffff" .. newStr .. "|r |cff777777(+0)|r"
                        tooltip:AddDoubleLine(cleanName, rightText, 1, 0.82, 0)
                    end
                end
            end
